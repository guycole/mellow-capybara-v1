...
def test_validate_v2_payload_accepts_sample() -> None:
    loader = Loader(DummyPostGres())
    payload = _load_sample_v2_payload()

    assert loader.validate_v2_payload(payload, payload["fileName"])
...
def test_validate_v2_payload_rejects_v1_shape() -> None:
    ...
    assert not loader.validate_v2_payload(legacy_payload, legacy_payload["fileName"])
